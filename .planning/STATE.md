...
| 07 | verification_deferred_human | /gsd-verify-work 7 |

Phase 07 is code-complete (4/4 plans; all 9 D7 must-haves code-verified 9/9 against the actual codebase by gsd-verifier; 174/174 tests pass, lint clean; executed sequentially on master — worktree isolation skipped to avoid the documented multi-wave base-divergence). Chat-type contexts (NIP-29 groups + DMs) now get default per-context notification rate limits: a pure per-context accounting layer (`contexts` map keyed `type:contextKey`, lazily created, window-pruned; most-restrictive-wins `underContext` gate in `evaluate()`), new `perGroup`/`perDm` config defaults (3/5, 0 = unlimited) with migration backfill + kind-30078 sync (PREFS_VERSION → 4), the per-context key threaded end-to-end through `rateLimitedNotify` (group site → `encodeGroupPointer`, both DM sites → raw `sender` for a shared per-counterparty bucket; `replies.ts`/`zaps.ts` untouched, flush timer byte-identical), and two clamped default-limit UI fields on /groups + /messages. Two UAT checks require a running UI (field render/PATCH-save/reload persistence) and a live signer (kind-30078 sync round trip + old-peer fallback-to-local-defaults) — deferred consistent with the Phase 1–6 pattern. Resume with `/gsd-verify-work 7` when a signer/UI is available.
